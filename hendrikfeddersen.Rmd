...
```{r eval=FALSE}
Time.spent.on.appraisal <- read_csv("https://https://hranalytics.netlify.com/data/Time.spent.on.appraisals.csv")
```

```{r read_appraisals_data, echo=FALSE, warning=FALSE, message=FALSE}
Time.spent.on.appraisal <- read_csv("data/Time.spent.on.appraisals.csv")
```
...
```{r}
"../2 stop appraisals/Time.spent.on.appraisals.csv" %>% 
  read_csv() ->
  Time.spent.on.appraisals
...
write_csv(results, "commute_times.csv")
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
results = read_csv("commute_times.csv")
```
...
ggsave("~/Documents/GitHub/HR_Analytics/7 organisational network/plotshiringroup.pdf", width = 21, height = 29.7, units = "cm")
...
ggsave("~/Documents/GitHub/HR_Analytics/7 organisational network/plotshirin.pdf", width = 21, height = 29.7, units = "cm")
```
...
"../8 job classification/Deploydata.csv" %>% 
  read_csv() ->
  DeployDataset
...
![UCI logo](_images/UCI_logo.gif)

It needs to be loaded first and let us preview it.

```{r}
"../9 masking HR data/fertility_Diagnosis.txt" %>% 
...
![plot of double exponential probability density function](_images/dexpdf.gif)
...
Testingdatasetandpredictions <- testing %>% add_predictions(lmFit1)
...
Absence2016Data <- Absence2016Data %>% add_predictions(lmFit1)
...
```{r}
absent <- as.data.frame( absenteeism_at_work_factored %>% select(everything()) %>% filter(`Absenteeism time in hours` > 0))

season1 <- as.data.frame(absent %>% group_by(Seasons) %>% summarise(count= n(), percent = round(count*100/nrow(absent),1)) %>% arrange(desc(count)))
...
for(i in 1:length(colnames(interview_attendance))){
  ...
  outfile <- paste0("../21 interview attendance/codefiles_interview_attendance/", vvar, ".csv")
...
for(i in 1:length(colstomap)){
  j <- colstomap[i]
  vfilename <- paste0("../21 interview attendance/codefiles_interview_attendance/", colnames(interview_attendance)[j], ".csv")

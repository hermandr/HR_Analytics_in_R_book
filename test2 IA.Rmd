...
interview_attendance<- mutate_all(interview_attendance, list(tolower)) # Sets all words to lower case
...
for(i in 1:length(colnames(interview_attendance))){
  ...
  outdata <- interview_attendance %>% group_by(.dots = vvar) %>% dplyr::count(.dots = vvar)
...
for(i in 1:length(colstomap)){
  ...
  vto <- as.vector(dfcodes[,4])
    interview_attendance[,j] <- mapvalues(unlist(interview_attendance[,j]), from=vfrom, to=vto) #Items in interview_attendance[,j] that match items from will be replaced by items in to, matched by position.
  interview_attendance[,j] <- as.integer(unlist(interview_attendance[,j]))
